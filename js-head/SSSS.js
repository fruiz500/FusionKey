...
function displayshare(shares,quorum){
	...
	for (var i = 0; i < shares.length; i++) {
		...
		if(fileMode.checked){
			if(textMode.checked){
				output += "<br><br>" + '<a download="PL24p' + quorumStr + '.txt" href="data:,' + dataItem + '">PassLok 2.4 Part out of ' + quorumStr + ' as a text file</a>'
			}else{
				output += "<br><br>" + '<a download="PL24p' + quorumStr + '.plk" href="data:binary/octet-stream;base64,' + dataItem + '">PassLok 2.4 Part out of ' + quorumStr + ' as a binary file</a>'
			}
		}else{
			output += "<br>" + "<pre>" + ("PL24p" + quorumStr + "==" + dataItem + "==PL24p" + quorumStr).match(/.{1,80}/g).join("<br>") + "</pre>"
		}
	}
