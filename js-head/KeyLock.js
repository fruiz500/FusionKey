...
setTimeout(function(){									//do the rest after a short while to give time for the key screen to show
	...
	getSettings();
//	if(inviteBox.checked) sendMail();										//off for universal extension
...
//		if(firstInit) KeyDir = wiseHash(key,userName);
		...
		if(ChromeSyncOn && chromeSyncMode.checked){				//FusionKey gets the settings from sync, asynchronously
			var syncName = userName+'.myself';
    		chrome.storage.sync.get(syncName.toLowerCase(), function (obj) {
				var lockdata = obj[syncName.toLowerCase()];
				if(lockdata){											//the user isn't totally new: retrieve settings
...
//universal extension only: store keys in background page
	...
	focusBox();
	pasteMain()		//execute if there is something in main box
},30);
}
...
//	firstInit = false;
	setTimeout(function(){
		if(userName){
			mainMsg.textContent = 'Ready with identity: ' + userName
		}else{
			window.close()
		}
	},10)
...
function showLock(){
//	if(showLockBtn.textContent == 'Email'){		//no redirect to email in FusionKey
//		sendMail();
//		return
//	}
	callKey = 'showlock';
...
function extractLock(string){
		...
		if(possibleLock.length == 43 || possibleLock.length == 50){
			...
			for(var name in locDir){
				if(name == 'myself'){			//Lock for 'myself' is not in locDir
					if(myLockStr == possibleLock64){
						foundIndex = index	
					}					
				}else{
